...
Mustang::handleInput( void ) {
  ...
  while ( 1 ) {
    ...
    if (finished) {
        rc = (rc == LIBUSB_ERROR_TIMEOUT ? 0 : rc);
        break;
    }
    ...
    if ( 0==memcmp(read_buf,state_prefix,2) ) {
      ...
      switch( dsp_category ) {
        ...
        case 0x04:
        {
          ...
          strncpy_s( preset_names[idx], (const char *)read_buf+16, 32 );
...
Mustang::initialize( void ) {
  ...
  if ( init_value < 0 ) {
    ...
    fprintf( stdout, "Mustang V%d USB device found Vendor ID: 0x%4.4X - Product ID: 0x%4.4X\n", (isV2 ? 2 : 1), FENDER_VID, init_value);
  }

#ifndef _WIN32   // This functionality is not available on Windows
